...
fun toDomain(apiResponse: ApiResponse?): List<DomainRepo> {
    return apiResponse?.let { it -> it.items.map { toDomain(it) } } ?: emptyList()
}

private fun toDomain(source: ApiRepo): DomainRepo = with(source) {
    DomainRepo(this.full_name, toDomain(this.owner), this.description, this.updated_at,
        this.language, this.open_issues_count)
}
